...
def create_labels():
    dat=(pd.read_csv(os.path.join("label", "elevationlabel_40bin.csv"), header=None))
    return dat.to_numpy()
...
steps = 300
xgb = x.XGBClassifier(learning_rate=0.01, n_rounds=steps, objective="multi:softprob", num_class=8)
grid = gs(xgb, parameters, n_jobs=4, scoring="neg_log_loss", cv=3)
...
joblib.dump(model,'xgboost_elevation2_40bin.dat')
