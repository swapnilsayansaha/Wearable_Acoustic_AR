...
def create_labels():
    dat=(pd.read_csv(os.path.join("label", "azimuthlabels_40bin.csv"), header=None))
    return dat.to_numpy()
...
print(trainlabels.shape, trainlabel.shape)
parameters = {
     "eta": [0.01, 0.05, 0.10],
      "max_depth": [ 3, 15, 30, 45],
...
steps = 1000
xgb = x.XGBClassifier(learning_rate=0.01, n_rounds=steps, objective="multi:softprob", num_class=9)
grid = gs(xgb, parameters, n_jobs=4, scoring="balanced_accuracy", cv=3)
trainsm, labelsm = traindata, trainlabel

smote = sm(sampling_strategy='auto', k_neighbors=4, n_jobs=4)
trainsm, labelsm = smote.fit_sample(trainsm, labelsm)
print(trainsm.shape, labelsm.shape)

grid.fit(trainsm, labelsm)
model = grid.best_estimator_
joblib.dump(model,'xgboost_azimuth_40bin_final.dat')
preds = model.predict(testdata)
print(preds)
print(testlabel)
best_preds = ((np.asarray(preds)*40)-160)
